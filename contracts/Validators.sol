...
contract Validators is Params, SafeSend, IValidators {
    ...
    address payable constant burnReceiver =
        0x000000000000000000000000000000000000DaaA;
    ...
    modifier onlyRegistered() {
        ...
        require(
            votePools[_pool.validator()] == _pool,
            "Vote pool not registered"
        );
        _;
    }
    ...
    function initialize(
        ...
        require(
            _validators.length > 0 && _validators.length == _managers.length,
            "Invalid params"
        );
        ...
        for (uint8 i = 0; i < _validators.length; i++) {
            ...
            require(
                votePools[_validator] == IVotePool(0),
                "Validators already exists"
            );
            VotePool _pool = new VotePool(
                _validator,
                _managers[i],
                PERCENT_BASE,
                ValidatorType.Poa,
    ...
    function changeAdmin(
        address _newAdmin
    ) external onlyValidAddress(_newAdmin) onlyAdmin {
        admin = _newAdmin;
    ...
    function updateParams(
        ...
        require(
            _posBackup <= _posCount && _poaBackup <= _poaCount,
    ...
    function updateRates(
        uint _burnRate,
        uint _foundationRate
    ) external onlyAdmin {
        require(
            _burnRate.add(_foundationRate) <= PERCENT_BASE,
            "Invalid rates"
        );
    ...
    function updateFoundation(address payable _foundation) external onlyAdmin {
        foundation = _foundation;
        emit UpdateFoundationAddress(_foundation);
    }

    function withdrawFoundationReward() external onlyFoundation {
    ...
    function updateValidatorState(
        ...
        require(
            votePools[_validator] != IVotePool(0),
            "Corresponding vote pool not found"
        );
        votePools[_validator].switchState(pause);
    }

    /**
     * @dev Get current top N validators.
     * The NPoS engine will call this function to form authority validator set.
     *
     */
        ...
        onlyInitialized
    {
        ...
        if (backupValidators.length > 0) {
            ...
            for (uint8 i = 0; i < backupValidators.length; i++) {
                _totalBackupVote = _totalBackupVote.add(
                    votePools[backupValidators[i]].totalVote()
                );
            }
            ...
            if (_totalBackupVote > 0) {
                for (uint8 i = 0; i < backupValidators.length; i++) {
                    ...
                    uint256 _reward = _firstPart.mul(_pool.totalVote()).div(
                        _totalBackupVote
                    );
        ...
        if (activeValidators.length > 0) {
            uint _totalVote = 0;
            for (uint8 i = 0; i < activeValidators.length; i++) {
                _totalVote = _totalVote.add(
                    votePools[activeValidators[i]].totalVote()
                );
            }
            ...
            for (uint8 i = 0; i < activeValidators.length; i++) {
                ...
                if (_totalVote > 0) {
                    uint _secondPart = _pool
                        .totalVote()
                        .mul(_secondPartTotal)
                        .div(_totalVote);
     ...
     */
    function improveRanking() external override onlyRegistered {
        IVotePool _pool = IVotePool(msg.sender);
     ...
     */
    function lowerRanking() external override onlyRegistered {
        IVotePool _pool = IVotePool(msg.sender);
        require(_pool.state() == State.Ready, "Incorrect state");
     ...
     */
    function removeRanking() external override onlyRegistered {
        IVotePool _pool = IVotePool(msg.sender);
